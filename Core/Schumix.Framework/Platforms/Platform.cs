			...
			try
			{
				...
				else if(File.Exists("/etc/lsb-release"))
				{
					using(var file = new StreamReader("/etc/lsb-release"))
					{
						...
						distro = id.Substring(0, id.IndexOf(SchumixBase.NewLine));
						string release = s.Remove(0, s.IndexOf("DISTRIB_RELEASE=") + "DISTRIB_RELEASE=".Length);
						distro += SchumixBase.Space + release.Substring(0, release.IndexOf(SchumixBase.NewLine));
					}
				}
				...
				distro = distro.Replace(SchumixBase.NewLine.ToString(), string.Empty);
			}
			catch(Exception)
			{
