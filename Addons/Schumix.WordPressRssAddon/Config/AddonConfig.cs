...
namespace Schumix.WordPressRssAddon.Config
{
	sealed class AddonConfig
	{
		...
		public AddonConfig(string configfile)
		{
			try
			{
				Log.Debug("WordPressRssAddonConfig", ">> {0}", configfile);
		...
		private void Init(string configfile)
		{
			...
			int QueryTime = !xmldoc.SelectSingleNode("WordPressRssAddon/Rss/QueryTime").IsNull() ? Convert.ToInt32(xmldoc.SelectSingleNode("WordPressRssAddon/Rss/QueryTime").InnerText) : _querytime;
		...
		private bool IsConfig(string ConfigDirectory, string ConfigFile)
		{
			...
			else
			{
				...
				try
				{
					...
					w.WriteElementString("QueryTime", (!xmldoc.SelectSingleNode("WordPressRssAddon/Rss/QueryTime").IsNull() ? xmldoc.SelectSingleNode("WordPressRssAddon/Rss/QueryTime").InnerText : _querytime.ToString()));
