...
namespace Schumix.WordPressRssAddon.Commands
{
	class RssCommand : CommandInfo
	{
		...
		public void HandleWordPress(IRCMessage sIRCMessage)
		{
			...
			{
				var text = sLManager.GetCommandTexts("wordpress/info", sIRCMessage.Channel, sIRCMessage.ServerName);
				if(text.Length < 2)
				{
			...
			else if(sIRCMessage.Info[4].ToLower() == "list")
			{
				...
				if(!db.IsNull())
				{
					...
					foreach(DataRow row in db.Rows)
						sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetCommandText("wordpress/list", sIRCMessage.Channel, sIRCMessage.ServerName), row["Name"].ToString());
				}
				else
					sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetWarningText("FaultyQuery", sIRCMessage.Channel, sIRCMessage.ServerName));
			}
			...
			else if(sIRCMessage.Info[4].ToLower() == "change")
			{
				...
				else if(sIRCMessage.Info[5].ToLower() == "url")
				{
					...
					var db0 = SchumixBase.DManager.QueryFirstRow("SELECT Link FROM wordpressinfo WHERE LOWER(Name) = '{0}' And ServerName = '{1}'", sUtilities.SqlEscape(sIRCMessage.Info[6].ToLower()), sIRCMessage.ServerName);
					if(!db0.IsNull())
					{
