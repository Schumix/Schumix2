...
namespace Schumix.WordPressRssAddon.Commands
{
	partial class RssCommand : CommandInfo
	{
		...
		public void HandleWordPress(IRCMessage sIRCMessage)
		{
			...
			{
				var db = SchumixBase.DManager.Query("SELECT Name, Channel FROM wordpressinfo");
				if(!db.IsNull())
				{
					foreach(DataRow row in db.Rows)
					{
						...
						sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetCommandText("wordpress/info", sIRCMessage.Channel), name, channel.SplitToString(" "));
					}
				}
				else
			...
			{
				var db = SchumixBase.DManager.Query("SELECT Name FROM wordpressinfo");
				if(!db.IsNull())
				{
					...
					if(list == string.Empty)
						sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetCommandText("wordpress/list", sIRCMessage.Channel), SchumixBase.Space + sLConsole.Other("Nothing"));
					else
						sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetCommandText("wordpress/list", sIRCMessage.Channel), list);
				}
				else
					sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetWarningText("FaultyQuery", sIRCMessage.Channel));
			}
			...
			{
				var text = sLManager.GetCommandTexts("wordpress/start", sIRCMessage.Channel);
				if(text.Length < 3)
				{
				...
				foreach(var list in WordPressRssAddon.RssList)
				{
			...
			{
				var text = sLManager.GetCommandTexts("wordpress/stop", sIRCMessage.Channel);
				if(text.Length < 3)
				{
					sSendMessage.SendChatMessage(sIRCMessage, sLConsole.Translations("NoFound2", sLManager.GetChannelLocalization(sIRCMessage.Channel)));
					return;
			...
			else if(sIRCMessage.Info[4].ToLower() == "reload")
			{
				...
				if(sIRCMessage.Info[5].ToLower() == "all")
				{
					...
					sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetCommandText("wordpress/reload/all", sIRCMessage.Channel));
				}
				else
				{
					var text = sLManager.GetCommandTexts("wordpress/reload", sIRCMessage.Channel);
					if(text.Length < 2)
					{
						sSendMessage.SendChatMessage(sIRCMessage, sLConsole.Translations("NoFound2", sLManager.GetChannelLocalization(sIRCMessage.Channel)));
						return;
			...
			else if(sIRCMessage.Info[4].ToLower() == "channel")
			{
				...
				{
					var text = sLManager.GetCommandTexts("wordpress/channel/add", sIRCMessage.Channel);
					if(text.Length < 2)
					{
						sSendMessage.SendChatMessage(sIRCMessage, sLConsole.Translations("NoFound2", sLManager.GetChannelLocalization(sIRCMessage.Channel)));
						return;
					...
					var db = SchumixBase.DManager.QueryFirstRow("SELECT Channel FROM wordpressinfo WHERE Name = '{0}'", sUtilities.SqlEscape(sIRCMessage.Info[6].ToLower()));
					if(!db.IsNull())
					{
						...
						SchumixBase.DManager.Update("wordpressinfo", string.Format("Channel = '{0}'", data), string.Format("Name = '{0}'", sUtilities.SqlEscape(sIRCMessage.Info[6].ToLower())));
				...
				{
					var text = sLManager.GetCommandTexts("wordpress/channel/remove", sIRCMessage.Channel);
					if(text.Length < 2)
					{
						sSendMessage.SendChatMessage(sIRCMessage, sLConsole.Translations("NoFound2", sLManager.GetChannelLocalization(sIRCMessage.Channel)));
						return;
					...
					if(!db.IsNull())
					{
						...
						SchumixBase.DManager.Update("wordpressinfo", string.Format("Channel = '{0}'", data.Remove(0, 1, SchumixBase.Comma)), string.Format("Name = '{0}'", sUtilities.SqlEscape(sIRCMessage.Info[6].ToLower())));
						sSendMessage.SendChatMessage(sIRCMessage, text[0]);
					}
					else
						sSendMessage.SendChatMessage(sIRCMessage, text[1]);
