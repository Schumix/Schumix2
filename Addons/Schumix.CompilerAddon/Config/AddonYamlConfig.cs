...
namespace Schumix.CompilerAddon.Config
{
	sealed class AddonYamlConfig : AddonDefaultConfig
	{
		...
		{
			bool CompilerEnabled = (!nodes.IsNull() && nodes.ContainsKey("Enabled")) ? asd.ToBoolean(nodes["Enabled".ToYamlNode()].ToString()) : d_compilerenabled;
			...
			if(!nodes.IsNull() && nodes.ContainsKey("MaxAllocating"))
			{
				...
				Enabled = (!node2.IsNull() && node2.ContainsKey("Enabled")) ? asd.ToBoolean(node2["Enabled".ToYamlNode()].ToString()) : d_enabled;
				Memory = (!node2.IsNull() && node2.ContainsKey("Memory")) ? asd.ToInt32(node2["Memory".ToYamlNode()].ToString()) : d_memory;
			}
			...
			int WarningLevel = (!nodes.IsNull() && nodes.ContainsKey("WarningLevel")) ? asd.ToInt32(nodes["WarningLevel".ToYamlNode()].ToString()) : d_warninglevel;
			bool TreatWarningsAsErrors = (!nodes.IsNull() && nodes.ContainsKey("TreatWarningsAsErrors")) ? asd.ToBoolean(nodes["TreatWarningsAsErrors".ToYamlNode()].ToString()) : d_treatwarningsaserrors;
