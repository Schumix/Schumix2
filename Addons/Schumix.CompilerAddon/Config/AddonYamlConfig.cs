...
namespace Schumix.CompilerAddon.Config
{
	sealed class AddonYamlConfig : AddonDefaultConfig
	{
		...
		{
			bool CompilerEnabled = (!nodes.IsNull() && nodes.ContainsKey("Enabled")) ? nodes["Enabled".ToYamlNode()].ToBoolean() : d_compilerenabled;
			...
			if(!nodes.IsNull() && nodes.ContainsKey("MaxAllocating"))
			{
				...
				Memory = (!node2.IsNull() && node2.ContainsKey("Memory")) ? node2["Memory".ToYamlNode()].ToInt32() : d_memory;
			}
			...
			int WarningLevel = (!nodes.IsNull() && nodes.ContainsKey("WarningLevel")) ? nodes["WarningLevel".ToYamlNode()].ToInt32() : d_warninglevel;
			bool TreatWarningsAsErrors = (!nodes.IsNull() && nodes.ContainsKey("TreatWarningsAsErrors")) ? nodes["TreatWarningsAsErrors".ToYamlNode()].ToBoolean() : d_treatwarningsaserrors;
