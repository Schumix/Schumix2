...
namespace Schumix.ExtraAddon.Config
{
	sealed class AddonXmlConfig : AddonDefaultConfig
	{
		...
		public AddonXmlConfig(string configdir, string configfile)
		{
			...
			bool Enabled = !xmldoc.SelectSingleNode("ExtraAddon/Mode/Remove/Enabled").IsNull() ? asd.ToBoolean(xmldoc.SelectSingleNode("ExtraAddon/Mode/Remove/Enabled").InnerText) : d_enabled;
