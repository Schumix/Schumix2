		...
		public void HandleHg(IRCMessage sIRCMessage)
		{
			...
			else if(sIRCMessage.Info[4].ToLower() == "change")
			{
				...
				else if(sIRCMessage.Info[5].ToLower() == "url")
				{
					...
					var db1 = SchumixBase.DManager.QueryFirstRow("SELECT Link, Website FROM hginfo WHERE LOWER(Name) = '{0}' And ServerName = '{1}'", sIRCMessage.Info[6].ToLower(), sIRCMessage.ServerName);
					if(!db1.IsNull())
					{
