...
namespace Schumix.MantisBTRssAddon.Config
{
	sealed class AddonYamlConfig : AddonDefaultConfig
	{
		...
		public AddonYamlConfig(string configdir, string configfile)
		{
			...
			var rssmap = (yaml.Documents.Count > 0 && ((YamlMappingNode)yaml.Documents[0].RootNode).Children.ContainsKey("MantisBTRssAddon")) ? ((YamlMappingNode)((YamlMappingNode)yaml.Documents[0].RootNode).Children["MantisBTRssAddon".ToYamlNode()]).Children : (Dictionary<YamlNode, YamlNode>)null;
		...
		public bool CreateConfig(string ConfigDirectory, string ConfigFile)
		{
			...
			else
			{
				...
				try
				{
					...
					file.Write(nodes.Children.ToString("MantisBTRssAddon"));
					file.Close();
