...
namespace Schumix.MantisBTRssAddon.Commands
{
	class RssCommand : CommandInfo
	{
		...
		public void HandleMantisBT(IRCMessage sIRCMessage)
		{
			...
			{
				var text = sLManager.GetCommandTexts("mantisbt/info", sIRCMessage.Channel, sIRCMessage.ServerName);
				if(text.Length < 2)
				{
			...
			else if(sIRCMessage.Info[4].ToLower() == "list")
			{
				...
				if(!db.IsNull())
				{
					if(db.Rows.Count == 0)
					{
						sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetCommandText("mantisbt/list", sIRCMessage.Channel, sIRCMessage.ServerName), sLConsole.Other("Nothing", sLManager.GetChannelLocalization(sIRCMessage.Channel, sIRCMessage.ServerName)));
						return;
					}

					foreach(DataRow row in db.Rows)
						sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetCommandText("mantisbt/list", sIRCMessage.Channel, sIRCMessage.ServerName), row["Name"].ToString());
				}
				else
					sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetWarningText("FaultyQuery", sIRCMessage.Channel, sIRCMessage.ServerName));
			}
			...
			else if(sIRCMessage.Info[4].ToLower() == "change")
			{
				...
				else if(sIRCMessage.Info[5].ToLower() == "url")
				{
					...
					var db0 = SchumixBase.DManager.QueryFirstRow("SELECT Link FROM mantisbt WHERE LOWER(Name) = '{0}' And ServerName = '{1}'", sUtilities.SqlEscape(sIRCMessage.Info[6].ToLower()), sIRCMessage.ServerName);
					if(!db0.IsNull())
					{
					...
					var db1 = SchumixBase.DManager.QueryFirstRow("SELECT Link FROM mantisbt WHERE LOWER(Name) = '{0}' And ServerName = '{1}'", sUtilities.SqlEscape(sIRCMessage.Info[6].ToLower()), sIRCMessage.ServerName);
					if(!db1.IsNull())
					{
