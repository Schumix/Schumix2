...
namespace Schumix.MantisBTRssAddon.Commands
{
	public partial class RssCommand : CommandInfo
	{
		...
		public void HandleMantisBT(IRCMessage sIRCMessage)
		{
			...
			{
				var db = SchumixBase.DManager.Query("SELECT Name, Channel FROM mantisbt");
				if(!db.IsNull())
				{
					foreach(DataRow row in db.Rows)
					{
						...
						sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetCommandText("mantisbt/info", sIRCMessage.Channel), name, channel.SplitToString(" "));
					}
				}
				else
			...
			{
				var db = SchumixBase.DManager.Query("SELECT Name FROM mantisbt");
				if(!db.IsNull())
				{
					...
					if(list == string.Empty)
						sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetCommandText("mantisbt/list", sIRCMessage.Channel), sLConsole.Other("Nothing"));
					else
			...
			{
				var text = sLManager.GetCommandTexts("mantisbt/start", sIRCMessage.Channel);
				if(text.Length < 3)
				{
				...
				foreach(var list in MantisBTRssAddon.RssList)
				{
			...
			{
				var text = sLManager.GetCommandTexts("mantisbt/stop", sIRCMessage.Channel);
				if(text.Length < 3)
				{
					sSendMessage.SendChatMessage(sIRCMessage, sLConsole.Translations("NoFound2", sLManager.GetChannelLocalization(sIRCMessage.Channel)));
					return;
			...
			else if(sIRCMessage.Info[4].ToLower() == "reload")
			{
				...
				if(sIRCMessage.Info[5].ToLower() == "all")
				{
					...
					sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetCommandText("mantisbt/reload/all", sIRCMessage.Channel));
				}
				else
				{
					var text = sLManager.GetCommandTexts("mantisbt/reload", sIRCMessage.Channel);
					if(text.Length < 2)
					{
						sSendMessage.SendChatMessage(sIRCMessage, sLConsole.Translations("NoFound2", sLManager.GetChannelLocalization(sIRCMessage.Channel)));
						return;
			...
			else if(sIRCMessage.Info[4].ToLower() == "channel")
			{
				...
				{
					var text = sLManager.GetCommandTexts("mantisbt/channel/add", sIRCMessage.Channel);
					if(text.Length < 2)
					{
						sSendMessage.SendChatMessage(sIRCMessage, sLConsole.Translations("NoFound2", sLManager.GetChannelLocalization(sIRCMessage.Channel)));
						return;
					...
					var db = SchumixBase.DManager.QueryFirstRow("SELECT Channel FROM mantisbt WHERE Name = '{0}'", sUtilities.SqlEscape(sIRCMessage.Info[6].ToLower()));
					if(!db.IsNull())
					{
						...
						SchumixBase.DManager.Update("mantisbt", string.Format("Channel = '{0}'", data), string.Format("Name = '{0}'", sUtilities.SqlEscape(sIRCMessage.Info[6].ToLower())));
				...
				{
					var text = sLManager.GetCommandTexts("mantisbt/channel/remove", sIRCMessage.Channel);
					if(text.Length < 2)
					{
						sSendMessage.SendChatMessage(sIRCMessage, sLConsole.Translations("NoFound2", sLManager.GetChannelLocalization(sIRCMessage.Channel)));
						return;
					...
					if(!db.IsNull())
					{
						...
						SchumixBase.DManager.Update("mantisbt", string.Format("Channel = '{0}'", data.Remove(0, 1, SchumixBase.Comma)), string.Format("Name = '{0}'", sUtilities.SqlEscape(sIRCMessage.Info[6].ToLower())));
						sSendMessage.SendChatMessage(sIRCMessage, text[0]);
					}
					else
						sSendMessage.SendChatMessage(sIRCMessage, text[1]);
