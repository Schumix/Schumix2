...
namespace Schumix.SvnRssAddon.Config
{
	sealed class AddonXmlConfig : AddonDefaultConfig
	{
		...
		public AddonXmlConfig(string configdir, string configfile)
		{
			...
			int QueryTime = !xmldoc.SelectSingleNode("SvnRssAddon/Rss/QueryTime").IsNull() ? asd.ToInt32(xmldoc.SelectSingleNode("SvnRssAddon/Rss/QueryTime").InnerText) : d_querytime;
		...
		public bool CreateConfig(string ConfigDirectory, string ConfigFile)
		{
			...
			{
				Log.Error("SvnRssAddonConfig", sLConsole.GetString("No such config file!"));
				Log.Debug("SvnRssAddonConfig", sLConsole.GetString("Preparing..."));
				...
				try
				{
					...
					w.WriteElementString("QueryTime", (!xmldoc.SelectSingleNode("SvnRssAddon/Rss/QueryTime").IsNull() ? xmldoc.SelectSingleNode("SvnRssAddon/Rss/QueryTime").InnerText : d_querytime.ToString()));
					...
					Log.Success("SvnRssAddonConfig", sLConsole.GetString("Config file is completed!"));
				}
				catch(Exception e)
				{
					Log.Error("SvnRssAddonConfig", sLConsole.GetString("Failure was handled during the xml writing. Details: {0}"), e.Message);
				}
			}
