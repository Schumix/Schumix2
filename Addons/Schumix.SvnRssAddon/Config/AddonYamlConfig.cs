...
namespace Schumix.SvnRssAddon.Config
{
	sealed class AddonYamlConfig : AddonDefaultConfig
	{
		...
		public AddonYamlConfig(string configdir, string configfile)
		{
			...
			var rssmap = (yaml.Documents.Count > 0 && ((YamlMappingNode)yaml.Documents[0].RootNode).Children.ContainsKey("SvnRssAddon")) ? ((YamlMappingNode)((YamlMappingNode)yaml.Documents[0].RootNode).Children["SvnRssAddon".ToYamlNode()]).Children : YamlExtensions.NullYMap;
		...
		public bool CreateConfig(string ConfigDirectory, string ConfigFile)
		{
			...
			else
			{
				...
				try
				{
					...
					file.Write(nodes.Children.ToString("SvnRssAddon"));
					file.Close();
