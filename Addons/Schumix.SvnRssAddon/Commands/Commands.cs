		...
		public void HandleSvn(IRCMessage sIRCMessage)
		{
			...
			else if(sIRCMessage.Info[4].ToLower() == "list")
			{
				...
				if(!db.IsNull())
				{
					if(db.Rows.Count == 0)
					{
						sSendMessage.SendChatMessage(sIRCMessage, sLManager.GetCommandText("svn/list", sIRCMessage.Channel, sIRCMessage.ServerName), sLConsole.Other("Nothing"));
						return;
					}
			...
			else if(sIRCMessage.Info[4].ToLower() == "add")
			{
				...
				var db = SchumixBase.DManager.QueryFirstRow("SELECT 1 FROM svninfo WHERE LOWER(Name) = '{0}' And ServerName = '{1}'", sUtilities.SqlEscape(sIRCMessage.Info[5].ToLower()), sIRCMessage.ServerName);
				if(db.IsNull())
				{
					bool started = false;
