		...
		public void HandleSvn(IRCMessage sIRCMessage)
		{
			...
			else if(sIRCMessage.Info[4].ToLower() == "channel")
			{
				...
				if(sIRCMessage.Info[5].ToLower() == "add")
				{
					...
					var db = SchumixBase.DManager.QueryFirstRow("SELECT Channel FROM svninfo WHERE Name = '{0}' And ServerName = '{1}'", sUtilities.SqlEscape(sIRCMessage.Info[6]), sIRCMessage.ServerName);
					if(!db.IsNull())
					{
						...
						SchumixBase.DManager.Update("svninfo", string.Format("Channel = '{0}'", data), string.Format("Name = '{0}' And ServerName = '{1}'", sUtilities.SqlEscape(sIRCMessage.Info[6]), sIRCMessage.ServerName));
				...
				else if(sIRCMessage.Info[5].ToLower() == "remove")
				{
					...
					if(!db.IsNull())
					{
						...
						SchumixBase.DManager.Update("svninfo", string.Format("Channel = '{0}'", data.Remove(0, 1, SchumixBase.Comma)), string.Format("Name = '{0}' And ServerName = '{1}'", sUtilities.SqlEscape(sIRCMessage.Info[6]), sIRCMessage.ServerName));
						sSendMessage.SendChatMessage(sIRCMessage, text[0]);
					}
					else
						sSendMessage.SendChatMessage(sIRCMessage, text[1]);
