			...
			if(IsAdmin(Network.IMessage.Nick, Network.IMessage.Host, AdminFlag.Operator))
			{
				if(Network.IMessage.Info[4].ToLower() == "svn")
				{
					if(Network.IMessage.Info.Length < 6)
					{
						...
						sSendMessage.SendCMPrivmsg(Network.IMessage.Channel, "Svn parancsai: channel | info | lista | start | stop | reload");
						return;
					}
					...
					if(Network.IMessage.Info[5].ToLower() == "channel")
					{
						...
						if(Network.IMessage.Info[6].ToLower() == "add")
						{
							...
							sSendMessage.SendCMPrivmsg(Network.IMessage.Channel, "Használata: {0}svn channel add <rss neve> <csatorna>", IRCConfig.CommandPrefix);
						}
						else if(Network.IMessage.Info[6].ToLower() == "del")
						{
							...
							sSendMessage.SendCMPrivmsg(Network.IMessage.Channel, "Használata: {0}svn channel del <rss neve> <csatorna>", IRCConfig.CommandPrefix);
						}
					}
					...
					else if(Network.IMessage.Info[5].ToLower() == "start")
					{
						...
						sSendMessage.SendCMPrivmsg(Network.IMessage.Channel, "Használata: {0}svn start <rss neve>", IRCConfig.CommandPrefix);
					}
