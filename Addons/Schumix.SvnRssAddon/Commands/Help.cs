...
namespace Schumix.SvnRssAddon.Commands
{
	public partial class RssCommand : CommandInfo
	{
		public void Help(IRCMessage sIRCMessage)
		{
			...
			{
				if(sIRCMessage.Info[4].ToLower() == "svn")
				{
					if(sIRCMessage.Info.Length < 6)
					{
						sSendMessage.SendCMPrivmsg(sIRCMessage.Channel, "Svn rss-ek kezelése.");
						sSendMessage.SendCMPrivmsg(sIRCMessage.Channel, "Svn parancsai: channel | info | lista | start | stop | reload");
						return;
					}
					...
					if(sIRCMessage.Info[5].ToLower() == "channel")
					{
						...
						if(sIRCMessage.Info[6].ToLower() == "add")
						{
							...
							sSendMessage.SendCMPrivmsg(sIRCMessage.Channel, "Használata: {0}svn channel add <rss neve> <csatorna>", IRCConfig.CommandPrefix);
						}
						else if(sIRCMessage.Info[6].ToLower() == "del")
						{
							...
							sSendMessage.SendCMPrivmsg(sIRCMessage.Channel, "Használata: {0}svn channel del <rss neve> <csatorna>", IRCConfig.CommandPrefix);
						}
					}
					...
					else if(sIRCMessage.Info[5].ToLower() == "start")
					{
						...
						sSendMessage.SendCMPrivmsg(sIRCMessage.Channel, "Használata: {0}svn start <rss neve>", IRCConfig.CommandPrefix);
					}
					else if(sIRCMessage.Info[5].ToLower() == "stop")
					{
						sSendMessage.SendCMPrivmsg(sIRCMessage.Channel, "Rss leállítása.");
						sSendMessage.SendCMPrivmsg(sIRCMessage.Channel, "Használata: {0}svn stop <rss neve>", IRCConfig.CommandPrefix);
					}
					else if(sIRCMessage.Info[5].ToLower() == "reload")
					{
						if(sIRCMessage.Info.Length < 7)
						{
							...
							sSendMessage.SendCMPrivmsg(sIRCMessage.Channel, "Svn reload parancsai: all");
							return;
						}
						...
						else if(sIRCMessage.Info[6].ToLower() == "all")
						{
							...
							sSendMessage.SendCMPrivmsg(sIRCMessage.Channel, "Használata: {0}svn reload <rss neve>", IRCConfig.CommandPrefix);
						}
					}
				}
			}
