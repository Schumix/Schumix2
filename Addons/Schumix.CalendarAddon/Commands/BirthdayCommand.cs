	...
	class BirthdayCommand : CommandInfo
	{
		...
		public void HandleBirthday(IRCMessage sIRCMessage)
		{
			...
			if(sIRCMessage.Info[4].ToLower() == "info")
			{
				...
				if(!db.IsNull())
				{
					bool enabled = asd.ToBoolean(db["Enabled"].ToString());
					int year = asd.ToInt32(db["Year"].ToString());
					int month = asd.ToInt32(db["Month"].ToString());
					int day = asd.ToInt32(db["Day"].ToString());
