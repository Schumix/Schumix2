...
namespace Schumix.Server.Config
{
	sealed class XmlConfig : DefaultConfig
	{
		...
		public XmlConfig(string configdir, string configfile, bool colorbindmode)
		{
			...
			bool LogDateFileName = !xmldoc.SelectSingleNode("Server/Log/DateFileName").IsNull() ? xmldoc.SelectSingleNode("Server/Log/DateFileName").InnerText.ToBoolean() : d_logdatefilename;
			int LogMaxFileSize = !xmldoc.SelectSingleNode("Server/Log/MaxFileSize").IsNull() ? xmldoc.SelectSingleNode("Server/Log/MaxFileSize").InnerText.ToInt32() : d_logmaxfilesize;
			int LogLevel = !xmldoc.SelectSingleNode("Server/Log/LogLevel").IsNull() ? xmldoc.SelectSingleNode("Server/Log/LogLevel").InnerText.ToInt32() : d_loglevel;
			...
			int ListenerPort = !xmldoc.SelectSingleNode("Server/Server/Listener/Port").IsNull() ? xmldoc.SelectSingleNode("Server/Server/Listener/Port").InnerText.ToInt32() : d_listenerport;
			...
			bool Enabled = !xmldoc.SelectSingleNode("Server/Update/Enabled").IsNull() ? xmldoc.SelectSingleNode("Server/Update/Enabled").InnerText.ToBoolean() : d_updateenabled;
			...
			int MaxMemory = !xmldoc.SelectSingleNode("Server/Shutdown/MaxMemory").IsNull() ? xmldoc.SelectSingleNode("Server/Shutdown/MaxMemory").InnerText.ToInt32() : d_shutdownmaxmemory;

			new Framework.Config.ShutdownConfig(MaxMemory);

			bool Config = !xmldoc.SelectSingleNode("Server/Clean/Config").IsNull() ? xmldoc.SelectSingleNode("Server/Clean/Config").InnerText.ToBoolean() : d_cleanconfig;
			bool Database = !xmldoc.SelectSingleNode("Server/Clean/Database").IsNull() ? xmldoc.SelectSingleNode("Server/Clean/Database").InnerText.ToBoolean() : d_cleandatabase;

			new Framework.Config.CleanConfig(Config, Database);

			if(!xmldoc.SelectSingleNode("Server/Schumixs/Enabled").IsNull() ? xmldoc.SelectSingleNode("Server/Schumixs/Enabled").InnerText.ToBoolean() : d_schumixsenabled)
			{
