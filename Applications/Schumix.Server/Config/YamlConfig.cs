	...
	sealed class YamlConfig : DefaultConfig
	{
		...
		private void ServerMap(IDictionary<YamlNode, YamlNode> nodes)
		{
			...
			if(!nodes.IsNull() && nodes.ContainsKey("Listener"))
			{
				...
				ListenerPort = (!node.IsNull() && node.ContainsKey("Port")) ? node["Port".ToYamlNode()].ToInt32() : d_listenerport;
			}
		...
		{
			if((!nodes.IsNull() && nodes.ContainsKey("Enabled")) ? nodes["Enabled".ToYamlNode()].ToBoolean() : d_schumixsenabled)
			{
